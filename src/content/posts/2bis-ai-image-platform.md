...
summary: 用 FastAPI + Vue 3 + Redis 搭建一个支持文生图、参考图、编辑图、异步队列、体验积分、订阅额度和 API Key 控制台的全栈 AI 绘图平台，记录架构演进和踩坑经验
...
部署配置已有 Docker Compose、Nginx 和 systemd 的方案；存储目前用本地静态目录，S3 兼容对象存储还只是预留。测试覆盖了扣费、生成选项、上游客户端、Worker 恢复、历史记录和管理端 API Key。后续有适合公开的版本，再整理出来。
